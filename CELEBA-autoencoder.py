...
#         # Output size: [batch, 3, 32, 32]
#         self.encoder = nn.Sequential(
#             nn.Conv2d(3, 128, 4, stride=2, padding=1),            # [batch, 128, 16, 16]
#             # nn.BatchNorm2d(12),
#             nn.ReLU(),
...
#             # nn.BatchNorm2d(48),
#             nn.ReLU(),
# 			nn.Conv2d(512, 1024, 4, stride=2, padding=1),           # [batch, 1024, 2, 2]
#             nn.ReLU(),
#         )
#         self.linear_encoder = nn.Linear(1024*2*2, 768)  #[batch, 128]
#         self.linear_decoder = nn.Linear(768, 1024*4*4)  #[batch, 1024*4*4]
#         self.decoder = nn.Sequential(
#             # nn.ConvTranspose2d(96, 48, 4, stride=2, padding=1),  # [batch, 48, 4, 4]
#             nn.ReLU(),
...
#             # nn.BatchNorm2d(12),
#             nn.ReLU(),
#             nn.ConvTranspose2d(256, 3, 4, stride=2, padding=1),   # [batch, 3, 32, 32]
#             nn.Sigmoid(),
#         )
#
#     def forward(self, x):
...
#         encoded = self.linear_encoder(encoded.view(b_size, -1))
#
#         decoded = self.linear_decoder(encoded).view(-1, 1024, 4, 4)
#         decoded = self.decoder(decoded)
#
#         return encoded, decoded
...
#             # nn.ReLU(),
#         )
#         # self.linear_encoder = nn.Linear(48*4*4, 512)
#         # self.linear_decoder = nn.Linear(512, 48*4*4)
#         self.decoder = nn.Sequential(
#             # nn.ConvTranspose2d(96, 48, 4, stride=2, padding=1),  # [batch, 48, 4, 4]
#             # nn.ReLU(),
# 			nn.ConvTranspose2d(48, 24, 4, stride=2, padding=1),  # [batch, 24, 8, 8]
...
def main():
    ...
    testset = torchvision.datasets.CelebA(root='/data02/Atin/CELEBA/', split="train",
                                          download=True, transform=transform)
    trainset = torchvision.datasets.CelebA(root='/data02/Atin/CELEBA/', split='test',
                                           download=True, transform=transform)
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    torch.save(autoencoder.state_dict(), "./weights/autoencoder_CELEBA1_128.pkl")
